...
--
-- Hôte : db
-- Généré le : ven. 07 jan. 2022 à 12:01
...
INSERT INTO `Post` (`id_post`, `id_user`, `createdAt`, `title`, `image`, `content`) VALUES
...
INSERT INTO `User` (`id_user`, `role`, `email`, `name`, `password`) VALUES
...
ALTER TABLE `Comments`
  ...
  ADD KEY `link comment to post` (`id_post`),
  ADD KEY `link comment to user` (`id_user`);
...
ALTER TABLE `Post`
  ...
  ADD KEY `link user to post` (`id_user`);
...
--
ALTER TABLE `Comments`
  ADD CONSTRAINT `link comment to post` FOREIGN KEY (`id_post`) REFERENCES `Post` (`id_post`),
  ADD CONSTRAINT `link comment to user` FOREIGN KEY (`id_user`) REFERENCES `User` (`id_user`);

--
-- Contraintes pour la table `Post`
--
ALTER TABLE `Post`
  ADD CONSTRAINT `link user to post` FOREIGN KEY (`id_user`) REFERENCES `User` (`id_user`);
COMMIT;
